...
err_t tls_client_connected(void *arg, struct altcp_pcb *pcb, err_t err) {
	...
	//err_2 = altcp_write(state->pcb, TLS_CLIENT_HTTP_REQUEST_2, strlen(TLS_CLIENT_HTTP_REQUEST_2), TCP_WRITE_FLAG_COPY);
    if (err != ERR_OK) {
...
//boolean wificonnected here, sets true if connected and make another state? then goes to auto zero?
